...
local sdkKey = "X7B2oLeGUPJ415zWrW81F3"
...
local function get_agent_info(datafile)
	http_api.fetch({url = "https://cdn.optimizely.com/datafiles/"..datafile..".json", timeout = api_timeout}, fetch_callback)
end

local function agent_callback(result)
	if not result.completed then
		return
	end
	print("Printing result.data - "..result.data)
end

minetest.register_chatcommand("optagent", {
	description = "Talk to Optimizely Agent",
	func = function()
		http_api.fetch({url = "http://agent:8080/v1/config", timeout = api_timeout, extra_headers = {"X-Optimizely-SDK-Key"..sdkKey}}, agent_callback)
		return true
	end
})
